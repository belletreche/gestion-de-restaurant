<x-app-layout>
    <div class="mb-4">
        <h4 class="fw-bold mb-1">Nouveau Plat</h4>
        <p class="text-muted mb-0 small">Ajoutez un nouveau plat au menu</p>
    </div>
            ...
            <form action="{{ route('plats.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="nom" class="form-control form-custom @error('nom') is-invalid @enderror" value="{{ old('nom') }}" required>
                        @error('nom') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-semibold small">Prix (DA) <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" name="prix" class="form-control form-custom @error('prix') is-invalid @enderror" value="{{ old('prix') }}" required min="0.01">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="categorie_id" class="form-select form-custom @error('categorie_id') is-invalid @enderror">
                            ...
                            @foreach ($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('categorie_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="file" name="image" class="form-control form-custom @error('image') is-invalid @enderror" accept="image/*" required>
                        <div class="form-text small">JPG, JPEG, PNG, WEBP. Max 2 Mo.</div>
